...
if data_source == "Use Default Datasets":
    ...
    if uploaded_meltwater or uploaded_civicsignals or uploaded_openmeasure:
        ...
        # CivicSignals and Open-Measure use standard UTF-8
        ...
        with st.spinner("Combining uploaded datasets..."):
            df = combine_social_media_data(meltwater_df_upload, civicsignals_df_upload, openmeasure_df_upload)
...
with tab3:
    ...
    possible_text_cols = ['Hit Sentence', 'title', 'message', 'content', 'object_id']
    ...
    # Convert timestamp
    if 'Timestamp' not in risk_df.columns:
    ...
    try:
        G, pos, cluster_map = cached_network_graph(clustered_df if 'clustered_df' in locals() else risk_df)
        edge_trace = []
        for edge in G.edges():
            x0, y0 = pos[edge[0]]
            x1, y1 = pos[edge[1]]
