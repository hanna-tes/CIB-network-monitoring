...
        # Check if 'Influencer' column exists before plotting
...
            # Ensure `filtered_df_global` is not a slice
...
    # The final_preprocess_and_map_columns function now creates the required columns
...
            # Pass the correct DataFrame and a unique cache key
...
    
    # Use the coordination mode selected in the sidebar
    if coordination_mode == "Text Content":
...
                        colorscale='YlGnBu',
                        reversescale=True,
                        color=[cluster_map.get(n, -2) for n in G.nodes()],
                        size=10,
                        colorbar=dict(
                            thickness=15,
                            title='Cluster ID',
...
                node_trace.marker.size = [d['weight'] * 2 + 5 for n, d in G.degree(weight='weight')]
...
                    colorscale='YlGnBu',
                    reversescale=True,
                    color=[G.nodes[n].get('cluster', -2) for n in G.nodes()],
                    size=10,
                    colorbar=dict(
                        thickness=15,
                        title='Cluster ID',
